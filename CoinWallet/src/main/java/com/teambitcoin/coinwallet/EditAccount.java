...
public class EditAccount extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		TextView guidDisplay = (TextView) findViewById(R.id.settings_guid_textview); // display
																						// GUID
		...
		newQnAButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (!User.getLoggedInUser().authenticate(
						passwordField.getText().toString())) {
					Toast.makeText(getApplicationContext(),
							"Incorrect Password", Toast.LENGTH_SHORT).show();
					return;
				}
				User.getLoggedInUser().setQnA(
						questionField.getText().toString(),
						answerField.getText().toString());
				Toast.makeText(getApplicationContext(),
						"Security Question & Answer Changed", Toast.LENGTH_LONG)
						.show();
			}
		});
	}
