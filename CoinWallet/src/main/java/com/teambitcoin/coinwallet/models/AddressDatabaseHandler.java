...
public class AddressDatabaseHandler
{
	...
	public static String GetAddresseSQLInitQuery(){
		return "CREATE TABLE " + TABLE_NAME + " (" +
				...
				ADDRESS_BALANCE_NAME + " INTEGER NOT NULL," +
				ADDRESS_TOTAL_RECEIVED_NAME + " INTEGER NOT NULL);";
	}
	...
	public static ArrayList<Address> RetrieveAddresses(String userGuid, boolean isArchived){
		...
		String[] columns = { USER_ADDRESS_COLUMN_NAME, ADDRESS_LABEL_NAME, ADDRESS_TOTAL_RECEIVED_NAME ,ADDRESS_BALANCE_NAME} ;
		Cursor cursor = Database.query(TABLE_NAME, columns , USER_GUID_COLUMN_NAME + " = ? AND " + ADDRESS_ARCHIVED_MODE + " = ?", new String[]{userGuid, archiveModeValue}, null, null, null);
		...
		{	
			...
			{
				addressList.add(new Address(cursor.getString(addressIndex),
